...
export const TaskList = {
  ...
  render() {
    const tasks = TaskService.search(searchKeyword, currentType);
    const allDaily = TaskService.getByType('daily');
    const allLongterm = TaskService.getByType('longterm');
  ...
  _renderList(containerId, tasks, type, isActive) {
    const container = document.getElementById(containerId);
    if (!container) return;

    // 如果不是当前 tab 的类型，只用全量数据来判断空状态
    const filtered = isActive ? tasks : TaskService.search(searchKeyword, type);
    container.innerHTML = '';
